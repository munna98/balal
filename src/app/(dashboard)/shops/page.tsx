...
export default async function ShopsPage() {
  ...
  const paymentBalanceByShop = new Map<string, number>()
  for (const a of payments) {
    ...
    paymentBalanceByShop.set(a.shop_id, (paymentBalanceByShop.get(a.shop_id) ?? 0) + delta)
  }

  const totalSales = shops.reduce((sum, s) => sum + s._count.sales, 0)
  const totalOutstanding = shops.reduce((sum, s) => sum + (paymentBalanceByShop.get(s.id) ?? 0), 0)
  ...
  return (
    <main className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Card size="sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-normal text-muted-foreground">Outstanding Payments</CardTitle>
          </CardHeader>
      ...
      {shops.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center gap-4 py-14 text-center">
            ...
            <div className="max-w-sm space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Create your first shop to record sales, customers, and Payments for that location.
              </p>
            </div>
            <Button asChild>
        ...
        <ul className="grid gap-3 sm:grid-cols-2">
          {shops.map((shop) => {
            const outstanding = paymentBalanceByShop.get(shop.id) ?? 0
                ...
                >
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 border-t border-foreground/10 pt-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-[0.625rem] font-medium uppercase tracking-wide text-muted-foreground">Payment bal.</p>
