...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!existing) return NextResponse.json({ data: null, error: 'Advance not found' }, { status: 404 })
    ...
    return NextResponse.json({ data: null, error: 'Failed to update advance' }, { status: 500 })
  }
}
