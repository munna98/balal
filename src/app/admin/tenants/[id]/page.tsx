...
export default async function AdminTenantDetailPage({
  ...
  })) as unknown as PaymentForAdminTenant[]

  const totalOutstanding = payments.reduce((sum, payment) => {
    const paid = Number(payment.amount_paid)
    const repaid = payment.amount_repaid ? Number(payment.amount_repaid) : 0
    ...
    <main className="space-y-6 p-4 md:p-6">
      ...
      <Card>
        ...
        <CardContent className="grid gap-2 text-sm md:grid-cols-3">
          ...
          <p><span className="font-medium">Total payments outstanding:</span> {totalOutstanding.toFixed(2)}</p>
        </CardContent>
      </Card>
